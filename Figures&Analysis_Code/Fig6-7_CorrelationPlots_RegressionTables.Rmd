...
---
```{r}
df_plot<- read_csv('/Users/maharamamurthy/Library/CloudStorage/GoogleDrive-maha10@stanford.edu/My Drive/VisualMeasuresAsLanguageAgnosticScreeners/Analysis_Repo/df_plot.csv')
df_plot<- df_plot %>% mutate(reading_outcome1 = wcj_lwi_ss)
```
...
coord_cartesian(ylim = c(35, 160)) + 
  ...
  theme_pubr()+ theme( text = element_text(size = 18)) + theme(axis.text.x = element_text(size = 18),  # Adjust font size of x-axis labels
        axis.text.y = element_text(size = 18), axis.title.y.left = element_text(size = 18), strip.text = element_text(size = 32)) + 
...
fig2c<- ggplot(data = df_all, aes(x = MPabilitySS, y = reading_outcome1)) +
 ...
 geom_point(aes(color = cELStatus), size = 2, alpha = .6) +
  ...
  scale_color_manual(values = c( "darkslategray", "ivory4"),breaks = c("EL", "EO"), 
                     labels = c("Spanish", "English"))+  
  theme_pubr(legend = "none") + theme( text = element_text(size = 18))+ theme(axis.text.x = element_text(size = 18),  # Adjust font size of x-axis labels
        axis.text.y = element_text(size = 18), axis.title.y.left = element_text(size = 18),  strip.text = element_text(size = 32), legend.text = element_text(size=32))+
coord_cartesian(ylim = c(35, 160))
fig2c
```
...
#### A reviewer is suggesting that the scaling and. centering be done after the group split 
 #For K first 
 # center the predictors 
 # Calculate the means of the predictor variables
 mean_LetAbilitySS <- mean(df_K$LetAbilitySS, na.rm = TRUE)
 mean_pseAbilitySS <- mean(df_K$pseAbilitySS, na.rm = TRUE)
 mean_MPabilitySS <- mean(df_K$MPabilitySS, na.rm = TRUE)
 # Center the predictor variables by subtracting the means
 df_K$LetAbilitySS_centered <- df_K$LetAbilitySS - mean_LetAbilitySS
 df_K$pseAbilitySS_centered <- df_K$pseAbilitySS - mean_pseAbilitySS
 df_K$MPabilitySS_centered <- df_K$MPabilitySS - mean_MPabilitySS
 
 # Zscore them 
 df_K$LetAbilitySS_z <- scale(df_K$LetAbilitySS, center = TRUE, scale = TRUE)
 df_K$pseAbilitySS_z <- scale(df_K$pseAbilitySS, center = TRUE, scale = TRUE)
 df_K$MPabilitySS_z <- scale(df_K$MPabilitySS, center = TRUE, scale = TRUE)
 df_K$reading_outcome1_z <- scale(df_K$reading_outcome1, center = TRUE, scale = TRUE)

#And then G1 

 # center the predictors 
 # Calculate the means of the predictor variables
 mean_LetAbilitySS <- mean(df_1$LetAbilitySS, na.rm = TRUE)
 mean_pseAbilitySS <- mean(df_1$pseAbilitySS, na.rm = TRUE)
 mean_MPabilitySS <- mean(df_1$MPabilitySS, na.rm = TRUE)
 # Center the predictor variables by subtracting the means
 df_1$LetAbilitySS_centered <- df_1$LetAbilitySS - mean_LetAbilitySS
 df_1$pseAbilitySS_centered <- df_1$pseAbilitySS - mean_pseAbilitySS
 df_1$MPabilitySS_centered <- df_1$MPabilitySS - mean_MPabilitySS
 
 # Zscore them 
 df_1$LetAbilitySS_z <- scale(df_1$LetAbilitySS, center = TRUE, scale = TRUE)
 df_1$pseAbilitySS_z <- scale(df_1$pseAbilitySS, center = TRUE, scale = TRUE)
 df_1$MPabilitySS_z <- scale(df_1$MPabilitySS, center = TRUE, scale = TRUE)
 df_1$reading_outcome1_z <- scale(df_1$reading_outcome1, center = TRUE, scale = TRUE)
...
# Splitting this for visibility of sample sizes with both the filters
...
#### ------- Need to scale and center again before you can proceed 
KG_Visual <- lm(reading_outcome1_z ~  LetAbilitySS_z+ pseAbilitySS_z + MPabilitySS_z , df_K)
summary(KG_Visual)
...
## A full model with FRPM and Primamry language 
KG_Full <- lm(reading_outcome1_z ~ LetAbilitySS_z* cProxySES*cELStatus+ pseAbilitySS_z*cProxySES*cELStatus + MPabilitySS_z* cProxySES*cELStatus , df_K)
summary(KG_Full)
G1_Full <- lm(reading_outcome1_z ~  LetAbilitySS_z* cProxySES * cELStatus + pseAbilitySS_z* cProxySES * cELStatus  + MPabilitySS_z* cProxySES * cELStatus  , df_1)
summary(G1_Full)
```
```{r}
...
fig3a<- ggplot(data = df_plot, aes(x = LetAbilitySS, y = reading_outcome1)) +
 ...
 fig3a <- fig3a+  geom_text(data = sample_sizesELi, aes(label = paste("n =", n)),
  ...
  theme_pubr()  + theme( text = element_text(size = 18),panel.background = element_rect(fill = "white")) + theme( text = element_text(size = 18)) + theme(axis.text.x = element_text(size = 18),  # Adjust font size of x-axis labels
        axis.text.y = element_text(size = 18), axis.title.y.left = element_text(size = 18), legend.position = "none", strip.text = element_text(size = 32), legend.text = element_text(size=25))
...
fig3b <- ggplot(data = df_plot, aes(x = pseAbilitySS, y = reading_outcome1)) +
 ...
 geom_point(aes(color = proxy_SES), size = 2, alpha = .5) +
  ...
  theme_pubr()  + theme( text = element_text(size = 18),panel.background = element_rect(fill = "white")) + theme( text = element_text(size = 18)) + theme(axis.text.x = element_text(size = 14),  # Adjust font size of x-axis labels
        axis.text.y = element_text(size = 18), axis.title.y.left = element_text(size = 18), legend.position = "none", strip.text = element_text(size = 32))
...
fig3c <- ggplot(data = df_plot, aes(x = MPabilitySS, y = reading_outcome1)) +
 ...
 geom_point(aes(color = proxy_SES), size = 2, alpha = .5) +
  ...
  theme_pubr(legend = "none")  + theme( text = element_text(size = 18),panel.background = element_rect(fill = "white")) + theme( text = element_text(size = 18)) + theme(axis.text.x = element_text(size = 18),  # Adjust font size of x-axis labels
        axis.text.y = element_text(size = 18), axis.title.y.left = element_text(size = 14),strip.text = element_text(size=32)) + facet_grid(~grade, labeller = labeller(grade = c(
    "0" = "K",
    "1" = "1",
    "2" = "2"))) 
