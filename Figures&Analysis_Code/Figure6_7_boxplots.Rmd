...
setwd('~/Library/CloudStorage/GoogleDrive-maha10@stanford.edu/My Drive/VisualMeasuresAsLanguageAgnosticScreeners/Analysis_Repo/')
df_demo_LET_Available <- read_csv("LET.csv")
df_demo_PSE_Available <- read_csv("PSE.csv")
df_demo_GMC_Available <- read_csv("GMC.csv")
```
...
bxp <- ggboxplot(
  ...
  facet_grid(~grade,labeller = labeller(grade = c(
    ...
    size = 5
        ...
        axis.text.y = element_text(size = 18), axis.title.y.left = element_text(size = 18), legend.position = "none", strip.text = element_text(size = 32),  #  Increase facet label (panel heading) font size
...
stat.test_adjusted <- stat.test2 %>%
  ...
  mutate(cohen.d = lapply(split(df_frmp_LET, df_frmp_LET$grade), function(x) {
    cohen.d(LetAbilitySS ~ relevel(as.factor(proxy_SES), ref = "Ineligible"), data = x)$estimate
  })) %>%
  mutate(cohen.d_effect = unlist(lapply(cohen.d, function(x) round(x, 3)))) %>%
  add_xy_position(x = "proxy_SES", dodge = 2.5)

stat.test <- stat.test_adjusted %>% add_xy_position(x = "proxy_SES",dodge =2.5)
bxp + 
  stat_pvalue_manual(stat.test,label="asteriks"  ,tip.length = 0.003,position = position_dodge(0.8), color = "gray2", size = 6) +
  stat_pvalue_manual(
    stat.test_adjusted, label = "cohen.d_effect", tip.length = 0.03,
    position = position_dodge(1.2), color = "gray2", size = 6, vjust = 1.5
  )
...
stat.test_adjusted <- stat.test2 %>%
  ...
  mutate(cohen.d = lapply(split(df_frpm_PSE, df_frpm_PSE$grade), function(x) {
    cohen.d(pseAbilitySS ~ relevel(as.factor(proxy_SES), ref = "Ineligible"), data = x)$estimate
  })) %>% 
  mutate(cohen.d_effect = unlist(lapply(cohen.d, function(x) round(x, 3)))) %>%
  add_xy_position(x = "proxy_SES", dodge = 2.5)
...
stat.test_adjusted <- stat.test2 %>%
  ...
  mutate(asteriks = ifelse(p <= alpha, "*", "ns")) %>%
   mutate(cohen.d = lapply(split(df_frpm_GMC, df_frpm_GMC$grade), function(x) {
    cohen.d(MPabilitySS ~ relevel(as.factor(proxy_SES), ref = "Ineligible"), data = x)$estimate
  })) %>%
  mutate(cohen.d_effect = unlist(lapply(cohen.d, function(x) round(x, 3)))) %>%
  add_xy_position(x = "proxy_SES", dodge = 2.5)
...
```{r}
# MEP-LET
df_primL_LET<- df_demo_LET_Available %>% filter(!(grade == 2| grade == 3)) %>% mutate(ELStatus = factor(ELStatus, levels = c("EO", "EL"))) 
...
bxp <- df_primL_LET %>% 
  ...
  ylab = "MEP-L (ø)", xlab = "" , fill = "ELStatus"
    ...
    "0" = "K",
    "1" = "1"))) + geom_text(
     data = sample_sizes %>% mutate(ordered_group = factor(ELStatus, levels = c("EO", "EL"))),
    aes(label = paste("n =", SampleSize),y=rep(-4.3,4), x = ordered_group),
    size = 5
...
stat.test_adjusted <- stat.test2 %>%
 ...
 mutate(cohen.d = lapply(split(df_primL_LET, df_primL_LET$grade), function(x) {
    cohen.d(LetAbilitySS ~ relevel(as.factor(ELStatus), ref = "EO"), data = x)$estimate
  })) %>%
  mutate(cohen.d_effect = unlist(lapply(cohen.d, function(x) round(x, 3)))) %>%
...
```{r}
# MEP-PSE
df_primL_PSE<- df_demo_PSE_Available %>% filter(!(grade == 2| grade == 3)) %>% mutate(ELStatus = factor(ELStatus, levels = c("EO", "EL"))) 
...
stat.test_adjusted <- stat.test2 %>%
  ...
  mutate(asteriks = ifelse(p <= alpha, "*", "ns")) %>%
   mutate(cohen.d = lapply(split(df_primL_PSE, df_primL_PSE$grade), function(x) {
    cohen.d(pseAbilitySS ~ relevel(as.factor(ELStatus), ref = "EO"), data = x)$estimate
  })) %>%
  mutate(cohen.d_effect = unlist(lapply(cohen.d, function(x) round(x, 3)))) %>%
  add_xy_position(x = "ELStatus", dodge = 2.5)
...
```{r}
# MP 
df_primL_GMC<- df_demo_GMC_Available %>% filter(!(grade == 2| grade == 3)) %>% mutate(ELStatus = factor(ELStatus, levels = c("EO", "EL"))) 
...
stat.test_adjusted <- stat.test2 %>%
  ...
  mutate(asteriks = ifelse(p <= alpha, "*", "ns")) %>%
   mutate(cohen.d = lapply(split(df_primL_GMC, df_primL_GMC$grade), function(x) {
    cohen.d(MPabilitySS ~ relevel(as.factor(ELStatus), ref = "EO"), data = x)$estimate
  })) %>%
  mutate(cohen.d_effect = unlist(lapply(cohen.d, function(x) round(x, 3)))) %>%
  add_xy_position(x = "ELStatus", dodge = 2.5)
...
stat.test_adjusted <- stat.test2 %>%
  mutate(p.adj = p/12) %>% mutate(asteriks = ifelse(stat.test2$p <= alpha, "*", ifelse(stat.test2$p <= 0.001,"***", "ns"))) %>%
   mutate(cohen.d = lapply(split(df_frpm_LET, df_frpm_LET$grade), function(x) {
    cohen.d(reading_outcome ~ relevel(as.factor(proxy_SES), ref = "Ineligible"), data = x)$estimate
  })) %>%
  mutate(cohen.d_effect = unlist(lapply(cohen.d, function(x) round(x, 3)))) %>%
  add_xy_position(x = "proxy_SES", dodge = 2.5)
...
bxp + 
  ...
  stat_pvalue_manual(
    stat.test_adjusted, label = "cohen.d_effect", tip.length = 0.03,
    position = position_dodge(1.2), color = "gray2", vjust = 1.5, size = 6
  )
...
stat.test_adjusted <- stat.test2 %>%
  mutate(p.adj = p/12) %>% mutate(asteriks = ifelse(stat.test2$p <= alpha, "*", ifelse(stat.test2$p <= 0.001,"***", "ns"))) %>%
   mutate(cohen.d = lapply(split(df_primL_LET, df_primL_LET$grade), function(x) {
    cohen.d(reading_outcome ~ relevel(as.factor(ELStatus), ref = "EO"), data = x)$estimate
  })) %>%
  mutate(cohen.d_effect = unlist(lapply(cohen.d, function(x) round(x, 3)))) %>%
  add_xy_position(x = "ELStatus", dodge = 2.5)
