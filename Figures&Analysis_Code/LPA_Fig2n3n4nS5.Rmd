...
set.seed(234543)
lpa_model <- Mclust(df_subset, g = 5,model ="EEI" )
...
df_createClusters <- df_with_clusters_plot %>%
    mutate(PerfGroups = case_when(
      RVP_LT0 > 0.25 & RVP_PT0 > 0.25 &
        ...
        RVP_LT0 > 0.25  &
            del_ucat_T0 > 0.25 & srt_ucat_T0 > 0.25  ~ "High Performers",
        RVP_LT0 < -0 &
...
ggplot(df_createClusters, aes(y = PerfGroups, x = wcj_lwi_ss_2024, fill = PerfGroups)) +
  ...
  scale_fill_manual(values = c( "cornsilk4","chocolate3", "cornflowerblue","aquamarine3","darkslategray4"),
...
cluster_means <- df_createClusters %>% #filter(PerfGroups == "Specific Visual Strength" | PerfGroups == "Specific Visual Challenge" ) %>% 
  ...
  group_by(PerfGroups) %>%
  summarise(across(c(ble_ucat_T0,del_ucat_T0, dgs_T0, nwr_ucat_T0,rao_T0, RVP_LT0, RVP_PT0,GMC_T0, srt_ucat_T0), mean, na.rm = TRUE)) %>%
...
profile_plot <- ggplot(cluster_means, 
     ...
     # Customize colors
     scale_color_manual(values = c("cornsilk4","darkslategray4", "chocolate3","cornflowerblue" ,"aquamarine3" 
                                 ),labels = paste0("", names(cluster_counts), 
                                  " (n=", cluster_counts, ")"),
                    name = "Cluster") +
...
ggplot() +
  ...
  labs(
   # title = "Reading Outcomes Over Time by Cluster",
    ...
    y = "WCJ-LWI (age standardized scores) "
  ) +
  theme_minimal() +
...
pairs(emmeans(model, ~ PerfGroups | time_point))
plot_data <- df_createClusters %>% 
  tidyr::pivot_longer(
#    cols = c(RO_T0, RO_T1),
...
```{r}
df_createClusters <- df_createClusters %>% 
  mutate(readingGrowth_LWI = wcj_lwi_2024 - wcj_lwi_2023) %>% 
  mutate(readingGrowth_WA = wcj_wa_2024 - wcj_wa_2023) 

# Color mapping to match the boxplot
cluster_colors <- c(
  "High Performers" = "darkslategray4",
  "Specific Visual Strength" = "aquamarine3",
  "Average Performers" ="chocolate3",
  "Specific Visual Challenge" = "cornflowerblue",
  "Low Performers" = "cornsilk4" 
)

create_half_violin <- function(data, column, x_label) {
  # Calculate medians for each cluster and order by median (descending)
  medians <- data %>%
    group_by(PerfGroups) %>%
    summarise(median = median(.data[[column]], na.rm = TRUE)) %>%
    arrange((median))  # Highest scores on top
  
  # Set factor levels based on ordered medians
  data$PerfGroups <- factor(data$PerfGroups, levels = medians$PerfGroups)
  medians$PerfGroups <- factor(medians$PerfGroups, levels = medians$PerfGroups)
  
  # Calculate 80th percentile
  percentile_80 <- quantile(data[[column]], 0.20, na.rm = TRUE)
  ...
  ggplot(data, aes(x = .data[[column]], y = PerfGroups, fill = PerfGroups)) +
    ...
    geom_vline(xintercept = percentile_80, 
               color = "gray", 
               linetype = "dashed", 
               size = 1, 
               alpha = 0.5) +
...
```{r}
df_createClusters <- df_createClusters %>%
  group_by(grade_2023) %>%  # assuming you have a grade column
  mutate(
    percentile_20_brs = quantile(wcj_lwi_ss_2024, 0.20, na.rm = TRUE),
    risk = ifelse(wcj_lwi_ss_2024 < percentile_20_brs, 1, 0)
  ) %>%
  ungroup()
...
# Order by percent_at_risk (descending) for the plot
risk_percentages <- risk_percentages %>%
  arrange(desc(percent_at_risk)) %>%
  mutate(PerfGroups = factor(PerfGroups, levels = PerfGroups))

# Define the same color mapping as the boxplot
# Map colors to the original cluster names
color_mapping <- c(
  "Low Performers" = "chocolate3",
  "Average Performers" = "cornsilk4", 
  "High Performers" = "darkslategray4",
  "Specific Visual Challenge" = "cornflowerblue",
  "Specific Visual Strength" = "aquamarine3"
)
...
# Create the plot
p1 <- ggplot(risk_percentages, aes(x = PerfGroups, y = percent_at_risk, fill = PerfGroups)) +
  geom_col() +
  geom_text(aes(label = sprintf("%.1f%%", percent_at_risk)),
            vjust = -0.5, size = 6) +
  scale_fill_manual(values = color_mapping) +
  labs(title = "Percentage of students at-risk by subgroup",
       subtitle = "Risk is scores below 20th percentile on WCJ-LWI a year-later",
       #x = "Cluster",
       y = "% at-risk") +
  theme_minimal() +
  theme(
...
##Supp FigS6 K
```{r}
# Step 1: Create a summary dataframe with proportions
df_summary <- df_createClusters %>% filter(grade_2023 == 0) %>% 
  group_by(PerfGroups, ELStatus) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(PerfGroups) %>%
  mutate(percentage = count / sum(count) * 100)  %>%
  ungroup() %>%
  mutate(PerfGroups = factor(PerfGroups, 
                              levels = c("High Performers", 
                                        "Specific Visual Strength", 
                                        "Average Performers", 
                                        "Specific Visual Challenge", 
                                        "Low Performers")))

# Step 2: Create the plot
plot5<- ggplot(df_summary, aes(x = factor(PerfGroups), y = percentage, fill = interaction(ELStatus))) +
  geom_bar(stat = "identity", position = "dodge") +
   scale_fill_manual(
    values = c("EL" = "darkslategray", "EO" = "ivory3"),  # green for EL, ivory for EO
    labels = c("EL" = "English Learner", "EO" = "English Only")  # optional: cleaner labels
  ) +
  labs(
    x = "Cluster",
    y = "Percentage",
    fill = "EL-FRPM Status",
    title = "Distribution of EL/EO and FRPM Status Within Each Cluster"
  ) +
   scale_y_continuous(
    breaks = seq(0, 100, 20),  # 0, 20, 40, 60, 80
    labels = scales::percent_format(scale = 1),
    limits = c(0, 100)  # optional: sets the axis range
  ) +
...
# Step 1: Create a summary dataframe with proportions
df_summary <- df_createClusters %>% filter(grade_2023 == 0) %>% 
  group_by(PerfGroups, frpm_eligibility) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(PerfGroups) %>%
  mutate(percentage = count / sum(count) * 100)  %>%
  ungroup() %>%
...
# Step 2: Create the plot
plot6<- ggplot(df_summary, aes(x = factor(PerfGroups), y = percentage, fill = interaction(frpm_eligibility))) +
  geom_bar(stat = "identity", position = "dodge") +
   scale_fill_manual(
    values = c("Eligible" = "skyblue", "Ineligible" = "slategray", "NotReported" = "gray"),  # green for EL, ivory for EO
    labels = c("Eligible" = "Eligible", "Ineligible" = "Ineligible", "NotReported" = "NA")  # optional: cleaner labels
  ) +
  labs(
    x = "Cluster",
    y = "Percentage",
...
cluster_means <- df_with_clusters_plot %>% #filter(PerfGroups == "Specific Visual Strength" | PerfGroups == "Specific Visual Challenge" ) %>% 
  #filter(PerfGroups == "High Performers" | PerfGroups == "Low Performers" | PerfGroups == "Average Performers") %>% 
  group_by(Cluster) %>%
  summarise(across(c(ble_ucat_T0,del_ucat_T0, dgs_T0, nwr_ucat_T0,rao_T0, RVP_LT0, RVP_PT0,GMC_T0, srt_ucat_T0), mean, na.rm = TRUE)) %>%
  tidyr::pivot_longer(-Cluster, names_to = "Predictor", values_to = "Mean")
# Create a named vector for label mapping
predictor_labels <- c(
  "evo_ucat_T0" = "Vocabulary", 
  "ble_ucat_T0" = "Blending",
...
```
## G1 
Now applying same cluster rules to G1 data 

```{r}
df_dum <- df_risk_imputed %>% dplyr::select(colsList_coh2_1)

df_subset <- df_risk_imputed[,colsList_coh2_1];
df_subset <- df_subset %>% filter( grade_2023 == 1)
columns_to_check <- c("RVP_LT0","RVP_PT0",  "del_ucat_T0", "evo_ucat_T0",
                      "nwr_ucat_T0", "dgs_T0", "rao_T0")
...
ggplot(df_createClusters, aes(y = PerfGroups, x = wcj_lwi_ss_2024, fill = PerfGroups)) +
  ...
  geom_vline(xintercept = percentile_20, color = "gray", linetype = "dashed", size = 1) +
  scale_fill_brewer(palette = "Set2") +
  scale_fill_manual(values = c( "chocolate3","cornflowerblue", "aquamarine3","cornsilk4","darkslategray4"),
                    labels = paste0(names(cluster_counts), 
                                  " (n=", cluster_counts, ")"),
                    name = "Cluster") +
  theme_minimal() +
...
```{r}
predictor_order <- c("evo_ucat_T0", "del_ucat_T0", "srt_ucat_T0",  "nwr_ucat_T0","dgs_T0", "rao_T0","GMC_T0", "RVP_LT0", "RVP_PT0" )
...
#Prim Lang 
# Step 1: Create a summary dataframe with proportions
df_summary <- df_createClusters %>% filter(grade_2023 == 1) %>% 
  group_by(PerfGroups, ELStatus) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(PerfGroups) %>%
  mutate(percentage = count / sum(count) * 100)  %>%
